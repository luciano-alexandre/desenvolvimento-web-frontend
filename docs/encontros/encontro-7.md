...
```bash
mkdir encontro-07-javascript
cd encontro-07-javascript
mkdir js
...
Prefira igualdade estrita, `===`, porque ela não converte os valores silenciosamente. As decisões tomadas a partir desses booleanos serão estudadas no Encontro 8.
...
O programa ainda não decide qual mensagem mostrar. Ele calcula condições e as exibe. No Encontro 8, `if`, `else if` e `else` transformarão esses booleanos em caminhos diferentes.
...
Esses fundamentos serão usados no Encontro 8 para criar caminhos de execução. As comparações que hoje apenas produzem `true` ou `false` passarão a controlar decisões com estruturas condicionais.
