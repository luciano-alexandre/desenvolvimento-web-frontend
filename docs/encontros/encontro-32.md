# Encontro 32 — Testes e qualidade

**Carga horária:** 1,5h  
**Entrega prevista:** Suíte inicial de testes

## Visão Geral

Este encontro desenvolve **testes e qualidade** como continuidade direta dos conhecimentos anteriores. A aula parte de um problema observável, apresenta os recursos necessários e termina com uma entrega que pode ser executada e verificada.
...
Teste de componente deve ser identificado no exemplo, aplicado na prática e relacionado ao resultado observado. Compare uma versão incompleta com a versão corrigida, explique a sintaxe relevante e registre quando esse recurso deve ou não ser utilizado.

### 2) Teste de serviço

Teste de serviço deve ser identificado no exemplo, aplicado na prática e relacionado ao resultado observado. Compare uma versão incompleta com a versão corrigida, explique a sintaxe relevante e registre quando esse recurso deve ou não ser utilizado.

### 3) Mocks

Mocks deve ser identificado no exemplo, aplicado na prática e relacionado ao resultado observado. Compare uma versão incompleta com a versão corrigida, explique a sintaxe relevante e registre quando esse recurso deve ou não ser utilizado.

### 4) Comportamento

Comportamento deve ser identificado no exemplo, aplicado na prática e relacionado ao resultado observado. Compare uma versão incompleta com a versão corrigida, explique a sintaxe relevante e registre quando esse recurso deve ou não ser utilizado.

### 5) Acessibilidade

Acessibilidade deve ser identificado no exemplo, aplicado na prática e relacionado ao resultado observado. Compare uma versão incompleta com a versão corrigida, explique a sintaxe relevante e registre quando esse recurso deve ou não ser utilizado.
...
```ts
it("deve emitir o identificador", () => {
  const spy = vi.fn();
  component.remover.subscribe(spy);
  component.solicitarRemocao();
...
**Proposta:** Testar uma regra e uma interação.
...
- demonstrar uso consciente de teste de componente;
- demonstrar uso consciente de teste de serviço;
- demonstrar uso consciente de mocks;
- demonstrar uso consciente de comportamento;
- demonstrar uso consciente de acessibilidade;
...
- [ ] Consigo explicar e aplicar **teste de componente**.
- [ ] Consigo explicar e aplicar **teste de serviço**.
- [ ] Consigo explicar e aplicar **mocks**.
- [ ] Consigo explicar e aplicar **comportamento**.
- [ ] Consigo explicar e aplicar **acessibilidade**.
...
Neste encontro, **testes e qualidade** foi tratado como parte de uma solução frontend completa. Conceitos, código, validação e comunicação técnica foram combinados para gerar um resultado reutilizável nos encontros seguintes e no projeto final.

## Questões de fixação

1. Como **teste de componente** contribui para a solução desenvolvida?
<!-- Gabarito: definir teste de componente, indicar sua finalidade e relacioná-lo ao exemplo e à prática. -->

2. Como **teste de serviço** contribui para a solução desenvolvida?
<!-- Gabarito: definir teste de serviço, indicar sua finalidade e relacioná-lo ao exemplo e à prática. -->

3. Como **mocks** contribui para a solução desenvolvida?
<!-- Gabarito: definir mocks, indicar sua finalidade e relacioná-lo ao exemplo e à prática. -->

4. Como **comportamento** contribui para a solução desenvolvida?
<!-- Gabarito: definir comportamento, indicar sua finalidade e relacioná-lo ao exemplo e à prática. -->

5. Como **acessibilidade** contribui para a solução desenvolvida?
<!-- Gabarito: definir acessibilidade, indicar sua finalidade e relacioná-lo ao exemplo e à prática. -->
